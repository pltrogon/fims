...
int main(int argc, char * argv[]) {
  ...
  //Lines generated along the y-axis of  the unit cell
  //The y-direction is the short axis of the geometry. 
  for(int i = 0; i < numFieldLine; i++){
    xStart.push_back(0.);
    yStart.push_back(yBoundary[0] + 2.*yBoundary[1]*i/(numFieldLine-1));
    }
  
  //Lines 
  //Right corner of unit cell
  xStart.push_back(rangeScale*(2./3.)*xBoundary[1]);
  yStart.push_back(0.);
  //Upper right corner of unit cell
  xStart.push_back(rangeScale*(1./3.)*xBoundary[1]);
  yStart.push_back(rangeScale*yBoundary[1]);
  //Middle of upper edge of unit cell    
  xStart.push_back(0.);
  yStart.push_back(rangeScale*yBoundary[0]);
