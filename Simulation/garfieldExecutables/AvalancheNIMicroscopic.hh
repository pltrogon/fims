#ifndef G_AVALANCHE_NIMICROSCOPIC_H
#define G_AVALANCHE_NIMICROSCOPIC_H
...
class AvalancheNIMicroscopic {
 public:
  /// Constructor
  AvalancheNIMicroscopic();
  /// Destructor
  ~AvalancheNIMicroscopic() {}
  ...
  /// Set Negative Ion Mass.
  void SetNegativeIonMass(int Z);
  double GetNegativeIonMass(){ return negativeionMass; };
  // Ion Dissution
  void EnableDiffusion() { m_useDiffusion = true; }
  void DisableDiffusion() { m_useDiffusion = false; }
  void EnableDetachment() { m_useDetachment = true; }
  void DisableDetachment() { m_useDetachment = false; }
  ...
  void SetTimeStep(const double d);
  void SetDistanceSteps(const double d);
  ...
  void GetElectronDriftLinePoint(double& x, double& y, double& z, double& t, int& changePoint,
                                 const int ip,
  ...
  bool AvalancheNegativeIon(const double x0, const double y0, const double z0,
...
//---------------------------------------------------------
  /// Detachment
  int DetachmentModelNo;
  // DetachmentModelNo = 0 -> Cross-Section Model
  // DetachmentModelNo = 1 -> Threshold Model
  void SetDetachModel(int model_num);
  void InputDetachCrossSectionData(std::string filename);
  void InputNegativeIonDiffusionData(std::string filename_dl,std::string filename_dt);
//---------------------------------------------------------

 private:
  std::string m_className = "AvalancheNIMicroscopic";

  Sensor* m_sensor = nullptr;
  
  double negativeionMass;
  double m_ionStep;
  double m_tMicroscopic;
  ...
  struct Electron {
    ...
    int particle;                  //< Electron=0 or hole=1 or NegativeIon=2.
  ...
  bool m_useDetachment = true;
  bool m_useDiffusionData = false;
  ...
  bool TransportNegativeIon(const double x0, const double y0, const double z0,
...
//---------------------------------------------------------
  int m_DetachCrossSectionPoint;
  std::vector<double> m_DetachData_xsec;
  std::vector<double> m_DetachData_xene;
  std::vector<double> m_DiffusionData_edl;
  std::vector<double> m_DiffusionData_dl;
  std::vector<double> m_DiffusionData_edt;
  std::vector<double> m_DiffusionData_dt;
  double GetDetachCrossSection(double energy);
  double ComputeLambda(double pressure, double x_sec);
  ...
  bool GetNegativeIonDiffusion(const double e, const double tmep, double& dl, double& dt);
  
  bool AddNIDiffusion(const int type, Medium* medium, const double step,
...
//---------------------------------------------------------
  ...
  void AddToStack(const double x, const double y, const double z,
                  const double t, const double energy, const int particle, const bool hole,
                  std::vector<Electron>& container) const;
  /// Add a new electron/hole to a container.
  void AddToStack(const double x, const double y, const double z,
                  const double t, const double energy, const int particle,
                  const double dx, const double dy, const double dz, const int band,
