...
#***********************************************************************************#
    def _writeSIF(self, mode):
        ...
        with open(filename, 'w') as f:
            f.write('! This file was generated by the FIMS code. Do not edit manually.\n\n')

            # --- Header ---
            headerData, simulationData = self._makeHeader(filename.split('.')[0])
            for key, value in headerData.items():
                f.write(f'\t{key} = {value}\n')
            f.write('End\n')

            # --- Simulation Data ---
            for key, value in simulationData.items():
                if isinstance(value, list):
                    valueStr = ' '.join(str(v) for v in value)
                    f.write(f'\t{key} = {valueStr}\n')
                else:
                    f.write(f'\t{key} = {value}\n')
            f.write('End\n')

            # --- Constants ---
            solverSettings, constants, equationSettings = self._makeSolver(filename.split('.')[0])
            for key, value in constants.items():
                f.write(f'\t{key} = {value}\n')
            f.write('End\n')

            # --- Solver Settings ---
            for key, value in solverSettings.items():
                if isinstance(value, list):
                    valueStr = ' '.join(str(v) for v in value)
                    f.write(f'\t{key} = {valueStr}\n')
                else:
            ...
            # --- Equation Settings ---
            for key, value in equationSettings.items():
                if isinstance(value, list):
                    valueStr = ' '.join(str(v) for v in value)
                    f.write(f'\t{key} = {valueStr}\n')
                else:
            ...
            materials, bodies = self._makeMaterials(filename.split('.')[0])
            ...
            # --- Boundary COnditions ---
            BCConfigs = self.makeBCs(filename.split('.')[0])
            for key, value in BCConfigs.items():
                if isinstance(value, list):
                    valueStr = ' '.join(str(v) for v in value)
                    f.write(f'\t{key} = {valueStr}\n')
                else:
...
#***********************************************************************************#
    def _makeHeader(self, mode):
        ...
        simulationData = {
            ...
            'Solver Input File': f'{mode}.sif',
            '!Post File': [f'{mode}.ep', f'{mode}.vtu'],
            'Output file': f'"elmerResults/{mode}.result"'
        }

        return headerData, simulationData

#***********************************************************************************#
    def _makeSolver(self, mode):
        ...
        if 'Capacitance' in mode:
            solverSettings['Calculate Capacitance Matrix'] = True
            solverSettings['Capacitance Matrix Filename'] = '"elmerResults/CapacitanceMatrix.dat"'
        ...
        return solverSettings, constants, equationSettings

#***********************************************************************************#
    def _makeMaterials(self, mode):
        ...
        return materials, bodies

#***********************************************************************************#
    def makeBCs(self, mode):
        """
        TODO
        """
        ...
        allModes = {
            ...
            'FIMSHexagonal': [
                'Cathode', 'Grid',
                'CentralPad', 'TopPad', 'BottomPad',
                'RightTopPad', 'RightBottomPad', 'LeftTopPad', 'LeftBottomPad',
                'MirrorBoundaries'
            ],
            'FIMSHexagonalCapacitance': [
                'Cathode', 'Grid',
                'CentralPad', 'TopPad', 'BottomPad',
                'RightTopPad', 'RightBottomPad', 'LeftTopPad', 'LeftBottomPad',
                'MirrorBoundaries'
            ...
            'FIMSSurrounding': [
                ...
                'CentralPad', 'TopPad', 'BottomPad',
                'RightTopPad', 'RightBottomPad', 'LeftTopPad', 'LeftBottomPad',
                'DielectricSurfaceCharge', 'MirrorBoundaries'
            ]
        }
        
        BCNames = allModes.get(mode, []) 

        BCConfigs = {}

        for i, name in enumerate(BCNames):
            key = f'Boundary Condition {i+1}'
            BCConfigs[key] = {
                'Target Boundaries(1)': i+1,
                'Name': f'"{name}"'
            }

            if name == 'MirrorBoundaries':
                BCConfigs[key]['Electric Flux'] = '0.0'
            elif name == 'DielectricSurfaceCharge':
                BCConfigs[key]['Charge Density'] = 'Variable Coordinate 1, Coordinate 2, Coordinate 3'
                BCConfigs[key]['File'] = '"chargeBuildup.dat"'
            elif 'Capacitance' in mode:
                BCConfigs[key]['Capacitance Body'] = i+1
            else:
                BCConfigs[key]['Potential'] = '0.0'
